...
HCPPIPEDIR=/data3/cnl/freesurfer/DCAN-HCP
...
# 3ddot /data3/cnl/ABCD_intermediates/abcd-hcp-pipeline/sub-0025427/ses-1/files/T1w/T1w_acpc_dc_restore_brain.nii.gz \
# /data3/cnl/xli/reproducibility/out/cpac_abcd/working/cpac_sub-0025427a_ses-1/get_restored_corrected_brain_90/sub-0025427a_run-1_T1w_resample_noise_corrected_corrected_warp_maths_maths_masked.nii.gz 
# >> 0.993974
# TODO add T1w_acpc_dc_restore (head) to RP

# 3ddot /data3/cnl/xli/reproducibility/out/cpac_abcd/working/cpac_sub-0025427a_ses-1/fast_bias_field_correction_52/get_anat_restore/sub-0025427a_run-1_T1w_resample_noise_corrected_corrected_warp_maths_restore_maths.nii.gz \ 
# /data3/cnl/ABCD_intermediates/abcd-hcp-pipeline/sub-0025427/ses-1/files/T1w/T1w_acpc_dc_restore.nii.gz
# 0.999856

# 3ddot /data3/cnl/xli/reproducibility/out/cpac_abcd/output/cpac_cpac_abcd-options/sub-0025427a_ses-1/anat/sub-0025427a_ses-1_space-template_desc-brain_T1w.nii.gz \
# /data3/cnl/ABCD_intermediates/abcd-hcp-pipeline/sub-0025427/ses-1/files/MNINonLinear/T1w_restore_brain.nii.gz
# >> 0.991752
# "$AtlasSpaceFolder"/"$AtlasSpaceT1wImage"

# TODO 
# 1. Add OutputInvTransform in CPAC
# https://github.com/DCAN-Labs/DCAN-HCP/blob/master/PostFreeSurfer/scripts/AtlasRegistrationToMNI152_ANTsbased.sh#L160
# https://github.com/FCP-INDI/C-PAC/blob/feature/ABCD_bold_mask/CPAC/registration/output_func_to_standard.py#L1086-L1379

# Questions:
# 1. How to organize working dir folder? - working dir
# 2. How to install MSM? https://github.com/ecr05/MSM_HOCR
# 3. How to evaluate surface outputs? - reg-diff
# 4. Where to hold DCAN templates/configs? 
...
for Hemisphere in L R ; do
  ...
  rm "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere".Strain_FS.native.shape.gii
	...
	if [ ${RegName} = "MSMSulc" ] ; then
		...
		# Note: the config is mordified according to acceptable args of msm version on Lisa - XL
		msm --conf=${MSMCONFIGDIR}/MSMSulcStrainFinalconf_lisa --inmesh="$AtlasSpaceFolder"/"$NativeFolder"/${Subject}.${Hemisphere}.sphere.rot.native.surf.gii --refmesh="$AtlasSpaceFolder"/"$Subject"."$Hemisphere".sphere."$HighResMesh"k_fs_LR.surf.gii --indata="$AtlasSpaceFolder"/"$NativeFolder"/${Subject}.${Hemisphere}.sulc.native.shape.gii --refdata="$AtlasSpaceFolder"/${Subject}.${Hemisphere}.refsulc."$HighResMesh"k_fs_LR.shape.gii --out="$AtlasSpaceFolder"/"$NativeFolder"/MSMSulc/${Hemisphere}. --verbose
    cp ${MSMCONFIGDIR}/MSMSulcStrainFinalconf_lisa "$AtlasSpaceFolder"/"$NativeFolder"/MSMSulc/${Hemisphere}.logdir/conf
		cd $DIR
